...
export class TcpServer {
  ...
  private sendHeartbeat(connectionId: string, state: ConnectionState): void {
    ...
    if (state.awaitingPong) {
      this.logger.warn('跳过心跳; 等待上一个pong', { connectionId });
      return;
    }
    ...
    this.logger.debug('发送心跳包中：', { connectionId });
    ...
    state.pongTimeout = setTimeout(() => {
      ...
      this.logger.warn('Pong 超时, 关闭连接', { connectionId });
      state.socket.destroy(new Error('心跳超时'));
  ...
  private handleHeartbeatAck(connectionId: string, state: ConnectionState): void {
    ...
    this.logger.debug('收到客户端 Pong 消息：', {
      connectionId,
      latency,
    });
  }
